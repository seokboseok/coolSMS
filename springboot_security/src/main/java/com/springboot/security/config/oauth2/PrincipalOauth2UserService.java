...
public class PrincipalOauth2UserService extends DefaultOAuth2UserService {
	...
	public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
		...
		Map<String, Object> attributes = oAuth2User.getAttributes();// 키값만 입력해주면 정보를 들고올수있다
		String provider = userRequest.getClientRegistration().getRegistrationId();// 무엇으로 로그인하지를 확인
		...
		if (userEntity == null) {// 처음로그인한것
			OAuth2UserDto oauth2userdto = OAuth2UserDto.builder().username(username)
					.password(new BCryptPasswordEncoder().encode(UUID.randomUUID().toString()))
					.email((String) attributes.get("email")).name((String) attributes.get("name")).role("ROLE_USER")
					.provider(provider).build();
			userEntity = oauth2userdto.toEntity();
			System.out.println(userEntity);
			userrepository.signup(userEntity);
		}
		return new PrincipalDetails(userEntity, attributes); // 세션에 담음
	}
}
